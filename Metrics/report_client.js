...
(function () {
    ...
    try {
        ...
        // ========================================
        // SCATTER PLOT: Time vs Memory (06-02)
        // ========================================
        function drawScatterPlot() {
            ...
            // Get Matrix 1 data for each language (most comparable)
            const scatterData = data
                .map(d => {
                    ...
                    return matrix1 ? {
                        solver: d.solver,
                        time: matrix1.time,
                        memory: matrix1.memory,
                        logo: d.logo || null
                    } : null;
                })
                .filter(d => d && d.time > 0 && d.memory > 0);

            // Identify outliers using window.scoringAnalysisData if available
            const outlierNames = new Set();
            if (window.scoringAnalysisData && window.scoringAnalysisData.outliers) {
                window.scoringAnalysisData.outliers.forEach(o => outlierNames.add(o.solver));
            }
            ...
            function redrawScatter() {
                ...
                // Create scales
                const xExtent = d3.extent(scatterData, d => d.time);
                const yExtent = d3.extent(scatterData, d => d.memory);

                const xScale = useLogScale
                    ? d3.scaleLog().domain([xExtent[0] * 0.5, xExtent[1] * 2])
                    : d3.scaleLinear().domain([0, xExtent[1] * 1.1]);
                xScale.range([margin.left, width - margin.right]);

                const yScale = useLogScale
                    ? d3.scaleLog().domain([yExtent[0] * 0.5, yExtent[1] * 2])
                    : d3.scaleLinear().domain([0, yExtent[1] * 1.1]);
                yScale.range([height - margin.bottom, margin.top]);

                // Draw axes
                g.append("g")
                    .attr("transform", `translate(0,${height - margin.bottom})`)
                    .call(d3.axisBottom(xScale).ticks(8).tickFormat(d => d < 1 ? d.toFixed(3) : d.toFixed(1)))
                    .selectAll("text")
                    .attr("fill", "#c0caf5")
                    .style("font-family", "JetBrains Mono");
                ...
                circles
                    .on("mouseover", function (event, d) {
                        ...
                        tooltip
                            .style("display", "block")
                            .html(`<strong>${d.solver}</strong><br/>Time: ${d.time.toFixed(3)}s<br/>Memory: ${d.memory.toFixed(1)} MB`);
                    ...
                    .on("click", function (event, d) {
                        ...
                        d3.selectAll(".scatter-point").classed("highlighted", false);
                        d3.selectAll("tr[data-lang]").style("background", "");
                        ...
                        if (row) {
                            row.style.background = "rgba(0, 255, 157, 0.15)";
            ...
            svg.append("foreignObject")
                ...
                .attr("height", 30)
                .append("xhtml:div")
                .html(`<button class="log-scale-toggle ${useLogScale ? 'active' : ''}" id="log-toggle">
                    ${useLogScale ? 'Log Scale' : 'Linear Scale'}
                </button>`)
                .on("click", function () {
                    useLogScale = !useLogScale;
                    d3.select("#log-toggle")
                        .classed("active", useLogScale)
                        .text(useLogScale ? "Log Scale" : "Linear Scale");
                    redrawScatter();
                });
        } // End of drawScatterPlot

        // ========================================
        // HEATMAP: Language x Matrix (06-02)
        // ========================================
        function drawHeatmap() {
            ...
            // Get all matrices and languages
            const matrices = ['1', '2', '3', '4', '5'];
            const languages = [...new Set(data.map(d => d.solver))].sort();

            // Build data matrix
            const heatmapData = [];
            languages.forEach(lang => {
                const langData = data.find(d => d.solver === lang);
                if (langData && langData.results) {
                    matrices.forEach(matrix => {
                        const result = langData.results.find(r => normalizeMatrix(r.matrix) === matrix);
            ...
            const times = heatmapData.map(d => d.time).filter(t => t > 0);
            ...
            g.append("g")
                ...
                .call(d3.axisTop(xScale).tickFormat(d => `M${d}`))
                .selectAll("text")
            ...
            gradient.append("stop")
                .attr("offset", "0%")
                .attr("stop-color", colorScale(d3.max(times)));

            gradient.append("stop")
                .attr("offset", "100%")
                .attr("stop-color", colorScale(d3.min(times)));
            ...
            legend.append("text")
                ...
                .attr("class", "heatmap-axis-label")
                .style("font-size", "10px")
                .text(`${d3.min(times).toFixed(3)}s`);
        } // End of drawHeatmap

        function showHeatmapDetail(data) {
            // Create modal overlay
            const overlay = d3.select("body").append("div")
                .attr("class", "modal-overlay visible")
                .style("display", "flex")
            ...
            modal.append("h3")
                ...
                .text(`${data.language} - Matrix ${data.matrix}`);
            ...
            details.append("div").html(`<strong>Time:</strong> ${data.time.toFixed(3)} seconds`);
            details.append("div").html(`<strong>Memory:</strong> ${data.memory.toFixed(1)} MB`);
            details.append("div").html(`<strong>Iterations:</strong> ${data.iterations ? data.iterations.toLocaleString() : 'N/A'}`);
            if (data.score) {
                details.append("div").html(`<strong>Score:</strong> ${data.score.toFixed(2)}`);
            }
        ...
        // ========================================
        // HISTOGRAM: Score Distribution (06-02)
        // ========================================
        function drawHistogram() {
            ...
            if (scores.length === 0) {
                d3.select("#d3-chart-container")
                    ...
                    .text("No score data available for histogram");
                return;
            }
            ...
            function drawPercentileLine(value, label, className) {
                const x = xScale(value);

                g.append("line")
                    .attr("class", `percentile-line ${className}`)
                    .attr("x1", x)
                    .attr("x2", x)
                    .attr("y1", margin.top)
                ...
                g.append("text")
                    .attr("class", `percentile-label ${className}`)
                    .attr("x", x)
                    .attr("y", margin.top - 10)
                    .text(label);
            }

            drawPercentileLine(q1, "Q1", "q1");
            drawPercentileLine(median, "Median", "median");
            drawPercentileLine(q3, "Q3", "q3");
            drawPercentileLine(mean, "Mean", "mean");
