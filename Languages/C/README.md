...
2.  Run the solver from this directory:
    ```bash
    docker run --rm -v "$(pwd)/../../..:/data" -w /data/Languages/C sudoku-content-server ./setupAndRunMe.sh [path_to_matrix_files]
    ```

    Example for a single matrix:
    ```bash
    docker run --rm -v "$(pwd)/../../..:/data" -w /data/Languages/C sudoku-content-server ./setupAndRunMe.sh ../../../Matrices/1.matrix
    ```
