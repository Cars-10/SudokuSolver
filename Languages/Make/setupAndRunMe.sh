...
# --- SETUP PHASE ---
if [ ! -f "Makefile" ]; then
    report_env_error "Makefile not found"
fi

# Ensure make is installed
if ! command -v make &> /dev/null; then
    report_env_error "make command not found"
fi
...
for matrix in $MATRICES; do
    ...
    # Generate matrix_data.mk
    echo "# Matrix Data" > matrix_data.mk
    row=0
    grep -v '^#' "$matrix" | while read -r line; do
        col=0
        for val in $line; do
    ...
    OS="$(uname)"
    if [ "$OS" = "Darwin" ]; then
        /usr/bin/time -l make solve INPUT="$matrix" > "$tmp_out" 2> "$tmp_err"
        ...
        /usr/bin/time -v make solve_matrix INPUT="$matrix" > "$tmp_out" 2> "$tmp_err"
