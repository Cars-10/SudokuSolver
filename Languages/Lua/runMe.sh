...
run_matrix() {
    ...
    if [ -n "$TIMEOUT_CMD" ]; then
        $TIMEOUT_CMD "$TIMEOUT_SECONDS" $TIME_CMD -f "%e %M %U %S" lua Sudoku.lua "$matrix_path" > "$temp_output" 2> "$temp_timing"
    else
        $TIME_CMD -f "%e %M %U %S" lua Sudoku.lua "$matrix_path" > "$temp_output" 2> "$temp_timing"
    fi
    local exit_code=$?
