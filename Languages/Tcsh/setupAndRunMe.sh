...
# --- SETUP PHASE ---
# No compilation needed for Tcsh
...
if ! command -v tcsh &> /dev/null; then
    report_env_error "tcsh command not found"
fi
...
for matrix in $MATRICES; do
    ...
    OS="$(uname)"
    if [ "$OS" = "Darwin" ]; then
        # Darwin /usr/bin/time -l provides memory in bytes
        /usr/bin/time -l tcsh Sudoku.tcsh "$matrix" > "$tmp_out" 2> "$tmp_err"
        ...
        # Linux /usr/bin/time -v provides memory in KB
        /usr/bin/time -v tcsh Sudoku.tcsh "$matrix" > "$tmp_out" 2> "$tmp_err"
