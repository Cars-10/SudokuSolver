...
report_env_error() {
    echo "[" > "$METRICS_FILE"
    echo "  {" >> "$METRICS_FILE"
    echo "    \"solver\": \"TypeScript\", " >> "$METRICS_FILE"
...
for matrix in $MATRICES; do
    ...
    OS="$(uname)"
    if [ "$OS" = "Darwin" ]; then
        /usr/bin/time -l npx ts-node Sudoku.ts "$matrix" > "$tmp_out" 2> "$tmp_err"
        ...
        /usr/bin/time -v npx ts-node Sudoku.ts "$matrix" > "$tmp_out" 2> "$tmp_err"
