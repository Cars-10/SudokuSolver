...
run_matrix() {
    ...
    # Use 'scala run' for Scala 3
    if [ -n "$TIMEOUT_CMD" ]; then
        $TIMEOUT_CMD "$TIMEOUT_SECONDS" $TIME_CMD -f "%e %M %U %S" scala run Sudoku.scala -- "$matrix_path" > "$temp_output" 2> "$temp_timing"
    else
        # No timeout command available, run without timeout
        $TIME_CMD -f "%e %M %U %S" scala run Sudoku.scala -- "$matrix_path" > "$temp_output" 2> "$temp_timing"
    fi
    local exit_code=$?
