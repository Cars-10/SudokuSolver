...
report_env_error() {
    echo "[" > "$METRICS_FILE"
    echo "  {" >> "$METRICS_FILE"
    echo "    \"solver\": \"Scala\", " >> "$METRICS_FILE"
...
# Compile/Setup
if [ -f "Sudoku.scala" ]; then
    scalac Sudoku.scala
    if [ $? -ne 0 ]; then
        report_env_error "Compilation failed"
    fi
else
    report_env_error "Sudoku.scala not found"
fi
...
for matrix in $MATRICES; do
    ...
    OS="$(uname)"
    if [ "$OS" = "Darwin" ]; then
        /usr/bin/time -l scala Sudoku "$matrix" > "$tmp_out" 2> "$tmp_err"
        ...
        /usr/bin/time -v scala Sudoku "$matrix" > "$tmp_out" 2> "$tmp_err"
