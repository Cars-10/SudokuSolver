/**
 * 3D Grid Renderer for Interactive Solver
 *
 * Renders the 9×9 sudoku grid with animations and visual effects.
 * Migrated from Metrics/modules/solver-grid.js to TypeScript
 */

import { componentRegistry } from '../../core/ComponentRegistry';
import type { SolverState, RenderOptions } from '../../types/solver';

export class SolverGridRenderer {
  private container: HTMLElement;
  private cells: HTMLElement[] = [];
  private gridContainer: HTMLElement | null = null;
  private initialPuzzle: number[][] | null = null;
  ...
  constructor(containerElement: HTMLElement) {
    ...
    componentRegistry.register({
      id: 'SGR',
      fullName: 'Solver Grid Renderer',
      type: 'solver',
      path: '/src/components/solver/SolverGrid.ts',
      dependencies: []
    });
  }
  ...
  initGrid(): void {
    this.container.innerHTML = `
      <div class="solver-grid-container" data-component-id="SGR-CONTAINER">
        <div class="solver-grid" data-component-id="SGR-GRID">
          ${Array(81).fill(0).map((_, i) => {
            const row = Math.floor(i / 9);
            const col = i % 9;
            return `
              <div class="solver-cell" data-row="${row}" data-col="${col}" data-component-id="SGR-CELL-${row}-${col}">
   ...
   */
  setInitialPuzzle(grid: number[][]): void {
  ...
  renderGrid(grid: number[][]): void {
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        if (valueSpan) {
          valueSpan.textContent = val === 0 ? '' : String(val);
          valueSpan.setAttribute('data-value', val === 0 ? '' : String(val));
        }
      }
    }
  }
   ...
   */
  render(state: SolverState, options: RenderOptions = {}): void {
    const { grid, row, col, value, isBacktrack } = state;
