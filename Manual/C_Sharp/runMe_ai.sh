...
for matrix in $PATTERN; do
    ...
    # Resolve absolute path for matrix file
    abs_matrix_path=$(cd "$(dirname "$matrix")"; pwd)/$(basename "$matrix")
    ...
    # Execute
    # Use --project to point to the Sudoku directory
    /usr/bin/time -l dotnet run --project Sudoku "$abs_matrix_path" > "$tmp_out" 2> "$tmp_err"
