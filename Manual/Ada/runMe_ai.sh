...
# Find gnatmake in alr toolchain
GNATMAKE=$(find ~/.local/share/alire/toolchains -name gnatmake -type f 2>/dev/null | head -1)

# Compile if source is newer than executable or executable doesn't exist
if [ ! -f ./sudoku ] || [ Sudoku.adb -nt sudoku ]; then
    echo "Compiling Ada solver..." >&2
    if [ -z "$GNATMAKE" ]; then
        echo "gnatmake not found in alr toolchain" >&2
        exit 1
    fi
    $GNATMAKE -O3 Sudoku.adb -o sudoku
    if [ $? -ne 0 ]; then
