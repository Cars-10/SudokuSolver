...
for matrix in $PATTERN; do
    ...
    # Execute
    # Using a dummy input to trigger END block or just passing file as argument
    # Awk processes files line by line, but our logic is in END block or triggered by file read
    # We pass the file as argument, but we also need to prevent awk from processing it line by line with default action
    # So we use an empty BEGIN/END structure or just rely on the script logic.
    # Actually, our script has an empty main block and logic in END.
    # But ARGV contains the file.
    # We need to run: awk -f Sudoku.awk file
