...
# The argument might be a glob pattern like "../../Matrices/[12].matrix"
# We need to let the shell expand it.
PATTERN="${1:-../../Matrices/*.matrix}"

# Check if gst is available
if ! command -v gst &> /dev/null; then
    echo "Error: gst (GNU Smalltalk) is not installed." >&2
    exit 1
fi
...
for matrix in $PATTERN; do
    ...
    # Execute
    # gst sudoku.st -a "$matrix"
    /usr/bin/time -l gst sudoku.st -a "$matrix" > "$tmp_out" 2> "$tmp_err"
