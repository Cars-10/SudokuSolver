...
for matrix in $PATTERN; do
    ...
    # Note: Cobol might still hang, but if it finishes, this will capture it.
    # We can add a timeout here if we want to be safe.
    # using 'gtimeout' or 'perl' for timeout if available, but for now standard execution.
