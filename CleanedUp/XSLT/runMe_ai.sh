...
for matrix in $PATTERN; do
    ...
    # PREPARE INPUT (wrap in XML)
    echo "<board>" > input.xml
    cat "$matrix" >> input.xml
    echo "</board>" >> input.xml
    
    # EXECUTION
    /usr/bin/time -l xsltproc Sudoku.xslt input.xml > "$tmp_out" 2> "$tmp_err"
