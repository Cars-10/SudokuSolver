...
report_env_error() {
    echo "[" > "$METRICS_FILE"
    echo "  {" >> "$METRICS_FILE"
    echo "    \"solver\": \"Elixir\", " >> "$METRICS_FILE"
...
if [ ! -f "Sudoku.exs" ]; then
    report_env_error "Sudoku.exs not found"
fi
...
for matrix in $MATRICES; do
    ...
    OS="$(uname)"
    if [ "$OS" = "Darwin" ]; then
        /usr/bin/time -l elixir Sudoku.exs "$matrix" > "$tmp_out" 2> "$tmp_err"
        ...
        /usr/bin/time -v elixir Sudoku.exs "$matrix" > "$tmp_out" 2> "$tmp_err"
