...
if [ ! -f "Sudoku.tcl" ]; then
    report_env_error "Sudoku.tcl not found"
fi
...
for matrix in $MATRICES; do
    ...
    OS="$(uname)"
    if [ "$OS" = "Darwin" ]; then
        /usr/bin/time -l tclsh Sudoku.tcl "$matrix" > "$tmp_out" 2> "$tmp_err"
        ...
        /usr/bin/time -v tclsh Sudoku.tcl "$matrix" > "$tmp_out" 2> "$tmp_err"
