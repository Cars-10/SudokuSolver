...
int readMatrixFile(char * filename ) {
    ...
    while (line_size >= 0)
    {
        ...
        if(line_buf[0] != '#' && line_buf[0] != '\n' && line_buf[0] != '\r') 
        {
            ...
            int parsed_count = sscanf ( line_buf , "%i %i %i %i %i %i %i %i %i", &puzzle[line_count][0], &puzzle[line_count][1], 
...
int isPossible(int y, int x, int val) {
    if (DEBUG) printf("Is possible %i, %i, %i\n" ,y ,x ,val);
    ...
    for (int i = 0; i <3; i++)
    {
        for (int j = 0; j <3; j++)
        {
            if (DEBUG>1) printf("Is Possible: y0+i=%i i=%i, x0+j=%i j=%i Puzzle[y0+i][x0+j]=%i, val=%i\n", y0+i,i, x0+j,j, puzzle[y0+i][x0+j] , val);
