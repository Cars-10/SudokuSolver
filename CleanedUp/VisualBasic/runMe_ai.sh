...
# -o bin: Output to bin directory
dotnet build -c Release -o bin > /dev/null 2>&1

if [ ! -f "bin/Sudoku" ] && [ ! -f "bin/Sudoku.dll" ]; then
    echo "Compilation failed"
    exit 1
fi

# Determine executable path
# Prefer dotnet command for reliability in this environment
EXEC="dotnet bin/Sudoku.dll"
...
for matrix in $PATTERN; do
    ...
    # Execute binary
    /usr/bin/time -l $EXEC "$matrix" > "$tmp_out" 2> "$tmp_err"
