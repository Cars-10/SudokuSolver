...
**Add dropdown option:**
Add `<option value="scatter">Time vs Memory</option>` to chart-selector (maintain alphabetical order after Task 1 in 06-01).
  </action>
  <verify>
...
**Add dropdown option:**
Add `<option value="heatmap">Heatmap</option>` to chart-selector (alphabetical order).
...
**Add dropdown option:**
Add `<option value="histogram">Score Distribution</option>` to chart-selector (alphabetical order).
  </action>
  <verify>
1. `npx ts-node Metrics/generate_report_only.ts`
...
3. Verify dropdown now includes: Algorithm Comparison, Heatmap, Horse Race, Histogram, Iteration Counts, Language, Metrics, Matrix Race, Score Distribution, Time vs Memory (alphabetical)
