...
1. **Create badge HTML generator helper (add to client-side JavaScript):**
```javascript
...
2. **Find the table row language name rendering location:**

The badge should be inserted in the language name cell at **line ~1252** in HTMLGenerator.ts.

**Exact location to find:** Search for `${displayName}${algoBadge}${typeIcon}` in HTMLGenerator.ts (around line 1252).

**Grep pattern to locate:** `grep -n 'displayName.*algoBadge.*typeIcon' Metrics/HTMLGenerator.ts`

This line currently renders the language display name with algorithm badge and type icon. Modify it to include the validation badge:
...
**Modified code:**
```javascript
${displayName}${algoBadge}${typeIcon}\${getValidationBadgeHtml('${lang}')}
```

**Important:** Since the table is server-side rendered but the validation badge needs to check window.validationIssues (client-side), use one of these approaches:

**Option A (Recommended):** Check validation issues at generation time in TypeScript:
```typescript
// In HTMLGenerator.ts, before the html += line:
const hasValidationIssue = validationIssues.some(i => i.language === lang);
const validationBadge = hasValidationIssue
    ? `<span class="validation-badge ${validationIssues.some(i => i.language === lang && i.severity === 'CRITICAL') ? 'critical' : 'warning'}" onclick="event.stopPropagation(); showDiagnosticsModal('${lang}')" title="Validation issues">${svgIcon}${validationIssues.filter(i => i.language === lang).length}</span>`
    : '';
```

Then add `${validationBadge}` after `${typeIcon}` in line 1252.

**Option B:** Use client-side JavaScript to add badges after page load via querySelectorAll on data-lang attributes.
...
1. `npx ts-node Metrics/generate_report_only.ts`
2. Open index.html in browser
3. Look for languages in the rankings table that have entries in benchmark_issues.json - should have warning badge
...
2. Open index.html in browser
3. Find a language with validation issues (check benchmark_issues.json for entries)
