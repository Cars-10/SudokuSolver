...
void dlx_cover_column(DlxColumn* c) {
    DlxNode *col_node = reinterpret_cast<DlxNode*>(c);
...
DlxColumn* choose_column(DlxColumn* root) {
    DlxNode *root_node = reinterpret_cast<DlxNode*>(root);
    DlxColumn *best = nullptr;
    int min_size = INT_MAX;

    DlxNode *col_node = root_node->right;
    while (col_node != root_node) {
        DlxColumn *col = reinterpret_cast<DlxColumn*>(col_node);
...
int dlx_search(DlxColumn* root, int k, int* solution) {
    ...
    DlxNode *row_node = (reinterpret_cast<DlxNode*>(col))->down;
    while (row_node != reinterpret_cast<DlxNode*>(col)) {
