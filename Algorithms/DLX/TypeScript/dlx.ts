...
const rowStarts: (DlxNode | null)[] = [];

// Calculate constraint column indices (matching C implementation)
function getPositionCol(r: number, c: number): number {
    return r * 9 + c;
}

function getRowCol(r: number, n: number): number {
    return 81 + r * 9 + (n - 1);
}

function getColCol(c: number, n: number): number {
    return 162 + c * 9 + (n - 1);
}

function getBoxCol(r: number, c: number, n: number): number {
    const box = Math.floor(r / 3) * 3 + Math.floor(c / 3);
    return 243 + box * 9 + (n - 1);
}

// Cover a column in the DLX matrix
function coverColumn(c: DlxColumn): void {
...
function uncoverColumn(c: DlxColumn): void {
    const colNode = c.node;
...
function chooseColumn(root: DlxColumn): DlxColumn | null {
...
function dlxSearch(root: DlxColumn, k: number, solution: number[]): boolean {
...
function initDlxMatrix(): { root: DlxColumn; columns: DlxColumn[] } {
    // Create root column
...
function addNode(col: DlxColumn, rowId: number): DlxNode {
...
function buildDlxRow(r: number, c: number, n: number, rowId: number, columns: DlxColumn[]): void {
    // Store row metadata
...
function buildDlxMatrixFromPuzzle(puzzle: number[][], columns: DlxColumn[]): void {
    let rowId = 0;
...
function coverClues(puzzle: number[][]): void {
    for (let r = 0; r < 9; r++) {
        for (let c = 0; c < 9; c++) {
            if (puzzle[r][c] !== 0) {
                const n = puzzle[r][c];
...
function extractSolution(solution: number[], solutionLen: number, puzzle: number[][]): number[][] {
    // Initialize solution grid - start with the original puzzle (includes clues)
    const solutionGrid: number[][] = puzzle.map(row => [...row]);
...
// Print puzzle
function printPuzzle(grid: number[][]): void {
    console.log("\nPuzzle:");
    for (let r = 0; r < 9; r++) {
...
// Read matrix file
function readMatrixFile(filename: string): number[][] {
