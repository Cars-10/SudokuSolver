...
interface CPGrid {
    values: number[][];      // Assigned values (0 = empty)
    candidates: number[][];  // Possible values per cell (bitset)
}

// Bitset helper functions
function hasCandidate(set: number, digit: number): boolean {
    return (set & (1 << digit)) !== 0;
}

function removeCandidate(set: number, digit: number): number {
    return set & ~(1 << digit);
}

function countCandidates(set: number): number {
...
function getFirstCandidate(cs: number): number {
...
function getPeers(row: number, col: number): number[][] {
    const peers: number[][] = [];
...
function initGrid(grid: CPGrid, puzzle: number[][]): void {
...
function eliminate(grid: CPGrid, row: number, col: number, digit: number): boolean {
...
function assign(grid: CPGrid, row: number, col: number, digit: number): boolean {
...
function propagate(grid: CPGrid): boolean {
    let changed = true;
...
function findMrvCell(grid: CPGrid): [number, number] | null {
...
function copyGrid(grid: CPGrid): CPGrid {
    return {
...
function cpSearch(grid: CPGrid, solution: number[]): boolean {
