#!/bin/bash
# Algorithms/CP/C_Sharp/runMe.sh - C# CP solver benchmark script
...
find_binary() {
    ...
    local binary=$(find bin/Release -name "CP.dll" -type f | sort -r | head -n 1)
    ...
    binary=$(find bin/Release -name "CP" -type f | sort -r | head -n 1)
    if [ -n "$binary" ]; then
        SOLVER_BINARY="./$binary"
        return
    fi

    report_env_error "Could not find compiled binary (CP.dll or CP)"
}
